from allocation.domain.commands import CreateBatch
from tests.unit.message_bus import FakeUnitOfWork


def test_add_batch_for_new_product(message_bus):
    uow = FakeUnitOfWork()
    message_bus.handle(CreateBatch("b1", "CRUNCHY-ARMCHAIR", 100, None), uow)
...
def test_add_batch_for_existing_product(message_bus):
    uow = FakeUnitOfWork()
    message_bus.handle(CreateBatch("b1", "GARISH-RUG", 100, None), uow)
